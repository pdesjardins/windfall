...
Combat in Windfall requires no special command. Attacking is an act of movement — when you move toward an enemy, the game resolves the engagement automatically.
...
Crew attack by attempting to move onto a hex occupied by enemy units. The outcome depends on who prevails:

- **Attacker wins:** The attacking crew moves onto the hex. Enemy units on that hex are destroyed.
- **Attacker is repulsed:** Both sides take damage. Both units remain on their original hexes.

## Durability

Different units absorb different amounts of damage before being destroyed:

**Crew** are destroyed by a single hit. However, a group of crew on one hex absorbs hits one at a time — each cannon shot or attack kills one crew member, leaving survivors unharmed.
...
Friendly units may stack on the same hex freely. Enemy units may never occupy the same hex as your units. Any attempt to move onto an enemy-occupied hex triggers combat instead.
